...
class DynamicStackDataset(Dataset):
    ...
    def sample_slice(self, sample_info: dict, n_frames: int = 50, start_idx: int | None = None):
        ...
        # 1. Normalización del módulo por la componente de frecuencia central (0, 0)
        dc_val = mod_tensor[:, 0:1, 0:1]
        mod_tensor = mod_tensor / (dc_val + 1e-8)

        # 2. Anulación de la frecuencia central (0, 0) tras normalizar
        mod_tensor[:, 0, 0] = 0.0
...
if __name__ == "__main__":
    ...
    try:
        ...
        save_dir = Path("/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_2channels_imageFFT_fixed_v4")
