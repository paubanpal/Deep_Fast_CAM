...
class DynamicStackDataset(Dataset):
    ...
    """
    def __init__(self, root_dir: str | Path, orig_data_path: str | Path, crop_dim: int | None = None, seed: int = 42):
        self.root_path = Path(root_dir)
        self.orig_data_path = Path(orig_data_path)
        ...
        for tel_dir in sorted(self.root_path.iterdir()):
            ...
            for module_file, phase_file in tel_fft_map.items():
                ...
                # Buscar la imagen original correspondiente en orig_data_path
                base_stem = module_file.split('_cropped')[0]
                orig_tel_dir = self.orig_data_path / tel_dir.name
    ...
    def sample_slice(self, sample_info: dict, n_frames: int = 50, start_idx: int | None = None):
        ...
        # Cargar la imagen original del telescopio si existe
        if orig_path and orig_path.exists():
            raw_orig = tiff.imread(orig_path)[start_idx : start_idx + n_frames].astype("float32")
            orig_max = raw_orig.max()
            orig_tensor = torch.tensor(raw_orig / (orig_max + 1e-8), dtype=torch.float32)
        else:
            orig_tensor = torch.tensor(mod_slice, dtype=torch.float32)
            orig_max = orig_tensor.amax()
        ...
        # Normalización del módulo de la imagen original por su valor máximo del stack
        orig_max_val = orig_tensor.amax(dim=(-2, -1), keepdim=True)
        orig_tensor_norm = orig_tensor / (orig_max_val + 1e-8)

        # Tensor de entrada de 2 canales [Módulo original normalizado, Fase original (o de FFT)] -> Shape: (N_frames, 2, H, W)
        input_2ch = torch.stack([orig_tensor_norm, phase_tensor], dim=1)
...
def evaluate_reconstruction_and_modes(model_path, data_path, orig_data_path, save_dir, device='cuda'):
    ...
    # 1. Cargar Dataset (Test en este caso)
    print("[INFO] Cargando dataset...")
    dataset = DynamicStackDataset(root_dir=data_path, orig_data_path=orig_data_path, seed=42)
    
    # 2. Inicializar y Cargar Modelo Entrenado (Una única vez)
    ...
    for test_idx, test_sample_info in enumerate(dataset.test_samples):
        print(f"\n[INFO] Procesando stack de test {test_idx + 1}/{len(dataset.test_samples)}: {test_sample_info['module_path'].name}")
        
        test_sample = dataset.sample_slice(test_sample_info, n_frames=50, start_idx=0)
        images_2ch = test_sample["images"].unsqueeze(0).to(device)     # [1, 50, 2, H, W]
        images_ft = test_sample["images_ft"].unsqueeze(0).to(device)   # [1, 50, H, W]
        cfg = test_sample["config"]
        ...
        base_name = Path(test_sample_info['module_path']).stem

        # --- TAREA 1: Reconstrucción del Objeto (Filtro de Wiener) y Búsqueda de Imagen Original ---
        print(f"[INFO] Generando gráfica y archivos del objeto reconstruido para {base_name}...")
        eps = 1e-6
        ...
        obj_tiff_path = save_dir / f"reconstructed_object_{base_name}.tiff"
        ...
        obj_single_png_path = save_dir / f"reconstructed_object_{base_name}.png"
        ...
        tel_name = test_sample_info["tel_dir_name"]
        mod_filename = test_sample_info["module_path"].name
        ...
        mean_tiff_path = save_dir / f"degraded_mean_input_{base_name}.tiff"
        ...
        mean_png_path = save_dir / f"degraded_mean_input_{base_name}.png"
        ...
        plt.suptitle(f"Reconstrucción - Stack: {test_sample_info['module_path'].name}", fontsize=14)
        plt.tight_layout()
        
        obj_plot_path = save_dir / f"inspection_reconstructed_object_{base_name}.png"
        ...
        kl_plot_path = save_dir / f"inspection_kl_modes_decay_{base_name}.png"
...
if __name__ == "__main__":
    ...
    try:
        ...
        output_dir = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_2channels_imagesFFT_fixed_v8_test/run_outputs_comprobacion_50/plots"
