...
class CNN(nn.Module):
    def __init__(self, n, n_lstm):
        ...
        # Entrada de 1 canal: Fase únicamente
...
class AugmentedDatasetWrapper:
    ...
    def augment(self, sample: dict) -> dict:
        ...
        # Recalcular la FFT compleja tras las transformaciones aplicando ifftshift (asumiendo módulo uniforme o recuperado si procede)
        mod_aug = torch.ones_like(sample["images"][:, 0, :, :])
...
def evaluate_reconstruction_and_modes(model_path, data_path, orig_data_path, save_dir, device='cuda'):
    ...
    # --- Guardar la PSF del primer fotograma (frame 0) ---
    print("[INFO] Extrayendo y guardando la PSF del primer fotograma...")

    # psf tiene shape (B * Nf, H, W) -> seleccionamos el fotograma 0
    psf_first_frame = psf[0]

    # Recentrar la componente DC al centro geométrico (deshacer convención FFT [0,0])
    psf_first_centered = torch.fft.fftshift(psf_first_frame, dim=(-2, -1)).cpu().numpy()

    # 1. Guardar en formato TIFF (32-bit float)
    first_psf_tiff_path = save_dir / "psf_frame_0.tiff"
    tiff.imwrite(first_psf_tiff_path, psf_first_centered.astype(np.float32))
    print(f"--> PSF (frame 0) guardada en TIFF: {first_psf_tiff_path}")

    # 2. Guardar en formato PNG lineal
    first_psf_png_path = save_dir / "psf_frame_0.png"
    plt.imsave(first_psf_png_path, psf_first_centered, cmap='viridis')
    print(f"--> PSF (frame 0) guardada en PNG lineal: {first_psf_png_path}")

    # 3. Guardar en formato PNG logarítmico (para visualizar los anillos de difracción / speckles débiles)
    first_psf_log_png_path = save_dir / "psf_frame_0_log.png"
    ...
    if matched_orig_path and matched_orig_path.exists():
        ...
        print(f"[WARNING] No se encontró coincidencia directa para '{base_stem}' en {orig_tel_dir}. Usando ceros o referencia alternativa.")
        degraded_mean = np.zeros((H, W))
    ...
    # Extraer el primer fotograma original degradado si existe, o el primer frame de entrada
    if matched_orig_path and matched_orig_path.exists():
        first_input_display = raw_orig_stack[0]
        panel0_title = "Fotograma 0 Original Degradado"
    else:
        first_input_display = images_1ch[0, 0, 0, :, :].cpu().numpy()
        panel0_title = "Fotograma 0 (Fase de Entrada)"

    fig, axes = plt.subplots(1, 3, figsize=(18, 6))

    axes[0].imshow(first_input_display, cmap='gray')
    ...
    im_psf = axes[2].imshow(np.log10(psf_first_centered + 1e-6), cmap='inferno')
    axes[2].set_title(r"PSF Frame 0 ($\log_{10}$)")
    axes[2].axis('off')
    fig.colorbar(im_psf, ax=axes[2], fraction=0.046, pad=0.04)

    plt.suptitle(f"Frame 0, Objeto y PSF - Stack: {val_sample_info['module_path'].name}", fontsize=14)
    plt.tight_layout()
    
    obj_plot_path = save_dir / "inspection_frame0_and_psf.png"
    plt.savefig(obj_plot_path, dpi=300)
    plt.close()
    print(f"--> Gráfica comparativa con PSF guardada exitosamente en: {obj_plot_path}")
...
if __name__ == "__main__":
    import traceback
    try:
        model_ckpt = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_1channel_multipFFT_phaseonly_fixed_v4/best_model.pt"
        ...
        output_dir = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_1channel_multipFFT_phaseonly_fixed_v4/run_outputs_comprobacion_50/plots"
