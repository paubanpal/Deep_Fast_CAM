...
def grid_search_variance(model_path, data_path, save_dir, variances=[1e-2, 1e-3, 5e-4, 1e-4, 1e-5], device='cuda'):
    """
    Evalúa la reconstrucción usando una serie de parámetros de varianza en el filtro de Wiener
    para estudiar la atenuación del halo alrededor de las estrellas sin alterar el modelo.
    """
    device = torch.device(device if torch.cuda.is_available() else 'cpu')
    print(f"[INFO] Usando dispositivo para Grid Search: {device}")
    
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)
    
    # 1. Cargar Dataset y muestra
    ...
    images = val_sample["images"].unsqueeze(0).to(device)
    ...
    dummy_var = torch.tensor([1e-3], dtype=torch.float32, device=device)
    lengths = torch.tensor([50], dtype=torch.int64, device=device)

    with torch.no_grad():
        _, num, den, _, _, _ = model(images_norm, images_ft, dummy_var, lengths=lengths)

    # 4. Generación de imágenes comparativas
    fig, axes = plt.subplots(1, len(variances) + 1, figsize=(4 * (len(variances) + 1), 4))
    
    degraded_mean = images[0].mean(dim=0).cpu().numpy()
    axes[0].imshow(degraded_mean, cmap='gray')
    axes[0].set_title("Media Entrada")
    axes[0].axis('off')

    eps = 1e-6
    for idx, var in enumerate(variances):
        obj_ft = num / (den.real + var + eps)
        obj_rec = torch.fft.ifft2(obj_ft, norm="ortho").real.squeeze().cpu().numpy()
        obj_rec = np.clip(obj_rec, a_min=0, a_max=None)

        # Guardar TIFF individual
        tiff.imwrite(save_dir / f"reconstructed_var_{var:.1e}.tiff", obj_rec.astype(np.float32))

        # Dibujar en el grid
        axes[idx + 1].imshow(obj_rec, cmap='gray')
        axes[idx + 1].set_title(f"var = {var:.1e}")
        axes[idx + 1].axis('off')

    plt.tight_layout()
    plot_path = save_dir / "study_variance_comparison.png"
    plt.savefig(plot_path, dpi=300)
    plt.close()
    print(f"--> Estudio comparativo de varianzas guardado exitosamente en: {plot_path}")
...
def evaluate_reconstruction_and_modes(model_path, data_path, save_dir, device='cuda'):
    ...
    # Restricción física de no-negatividad
...
if __name__ == "__main__":
    ...
    try:
        ...
        output_dir = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_mejoras_FFT_v2/run_outputs_comprobacion_50/plots"
        
        # 1. Ejecutar evaluación estándar
        evaluate_reconstruction_and_modes(model_ckpt, data_dir, save_dir=output_dir)
        
        # 2. Ejecutar estudio de varianzas para mitigar el halo
        grid_search_variance(model_ckpt, data_dir, save_dir=output_dir, variances=[1e-2, 1e-3, 5e-4, 1e-4, 1e-5])
