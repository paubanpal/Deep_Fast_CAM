...
class DynamicStackDataset(Dataset):
    ...
    def __init__(self, root_dir: str | Path, crop_dim: int | None = None, seed: int = 42):
        ...
        # Split 1 val and 1 test stack from EACH telescope, prioritizing stacks alphabetically by filename
        for tel_name, samples in telescope_samples.items():
            if len(samples) < 3:
                print(f"Warning: Telescopio {tel_name} tiene menos de 3 stacks válidos.")
                continue
                
            # Sort alphabetically by filename: deterministic selection for Val/Test
            sorted_samples = sorted(samples, key=lambda s: s["filename"])
...
if __name__ == "__main__":
    import traceback
    try:
        model_ckpt = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_2channels_divisFFT/best_model.pt"
        ...
        output_dir = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_2channels_divisFFT/run_outputs_comprobacion_50/plots"
