...
def evaluate_reconstruction_and_modes(model_path, data_path, orig_data_path, save_dir, device='cuda'):
    ...
    # --- TAREA EXTRA: Extraer y Guardar las PSFs correspondientes ---
    print("[INFO] Generando y guardando las PSFs...")

    # psf shape is (B * Nf, H, W) -> (50, H, W)
    # 1. Centrar el núcleo de la PSF (deshacer la convención FFT donde DC está en [0,0])
    psf_centered = torch.fft.fftshift(psf, dim=(-2, -1))
    psf_centered_np = psf_centered.cpu().numpy()

    # 2. PSF física media a lo largo de los 50 fotogramas
    mean_psf = np.mean(psf_centered_np, axis=0)

    # 3. Guardar la PSF media en TIFF (32-bit) y PNG (con escala logarítmica para ver los anillos de difracción)
    mean_psf_tiff_path = save_dir / "mean_psf.tiff"
    tiff.imwrite(mean_psf_tiff_path, mean_psf.astype(np.float32))

    mean_psf_png_path = save_dir / "mean_psf.png"
    plt.imsave(mean_psf_png_path, mean_psf, cmap='viridis')

    # Versión logarítmica (habitual para evaluar alas de la PSF y ruido de fondo)
    mean_psf_log_png_path = save_dir / "mean_psf_log.png"
    plt.imsave(mean_psf_log_png_path, np.log10(mean_psf + 1e-6), cmap='inferno')

    # # 4. (Opcional) Guardar todo el cubo de 50 PSFs instantáneas individuales
    # cube_psf_tiff_path = save_dir / "psf_stack_50frames.tiff"
    # tiff.imwrite(cube_psf_tiff_path, psf_centered_np.astype(np.float32))

    # 5. PSF efectiva del filtro de Wiener (Respuesta al impulso neta del proceso inverso)
    #    H_eff = sum(|S_k|^2) / (sum(|S_k|^2) + variance + eps)
    effective_otf = den.real / (den.real + variance[:, None, None] + eps)
    effective_psf_complex = torch.fft.ifft2(effective_otf, norm="ortho")
    effective_psf_centered = torch.fft.fftshift(effective_psf_complex, dim=(-2, -1)).real.squeeze().cpu().numpy()

    eff_psf_png_path = save_dir / "effective_wiener_psf.png"
    plt.imsave(eff_psf_png_path, effective_psf_centered, cmap='viridis')

    print(f"--> PSF media guardada en: {mean_psf_png_path}")
    print(f"--> PSF efectiva de Wiener guardada en: {eff_psf_png_path}")
    ...
    # Búsqueda del stack de imágenes originales correspondiente
    tel_name = val_sample_info["tel_dir_name"]
    mod_filename = val_sample_info["module_path"].name
    
    # Extraer identificador base del nombre (e.g. "CHR181", "wds15245", etc.)
    ...
    if matched_orig_path and matched_orig_path.exists():
        ...
        # Aplicar el mismo recorte central si la imagen original es más grande
    ...
    im_psf = axes[2].imshow(np.log10(mean_psf + 1e-6), cmap='inferno')
    axes[2].set_title(r"PSF Media ($\log_{10}$)")
    axes[2].axis('off')
    fig.colorbar(im_psf, ax=axes[2], fraction=0.046, pad=0.04)

    plt.suptitle(f"Reconstrucción y PSF - Stack: {val_sample_info['module_path'].name}", fontsize=14)
    plt.tight_layout()
    plt.savefig(save_dir / "inspection_reconstructed_with_psf.png", dpi=300)
    plt.close()
...
if __name__ == "__main__":
    import traceback
    try:
        model_ckpt = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_2channels_divisFFT_fixed_v2/best_model.pt"
        data_dir = "/scratch/paulabp/TFM/images/images_for_network/FFT/divis_FFTs"
        orig_data_dir = "/scratch/paulabp/TFM/images/images_for_network/originals_cropped"
        output_dir = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_2channels_divisFFT_fixed_v2/run_outputs_comprobacion_50/plots"
