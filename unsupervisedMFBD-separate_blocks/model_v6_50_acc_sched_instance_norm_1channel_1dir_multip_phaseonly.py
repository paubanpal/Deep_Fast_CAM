...
class DynamicStackDataset(Dataset):
    """
    Loads Phase stacks specified in mapping.json within root_dir.
    Supported JSON structure: "phase.tif" file mapping.
    """
    def __init__(self, root_dir: str | Path, crop_dim: int | None = None, seed: int = 42):
        ...
        for tel_dir in sorted(self.root_path.iterdir()):
            ...
            # Filter and keep only Phase files
            for k, v in tel_fft_map.items():
                phase_file = k if "phase" in k.lower() else (v if "phase" in v.lower() else None)
                if not phase_file:
                    continue
    ...
    def sample_slice(self, sample_info: dict, n_frames: int = 50, start_idx: int | None = None):
        ...
        # Complex reconstruction for Wiener filter using phase with unit magnitude
        fft_complex = torch.exp(1j * phase_tensor)
...
class AugmentedDatasetWrapper:
    ...
    def augment(self, sample: dict) -> dict:
        ...
        sample["images_ft"] = torch.exp(1j * phase_aug)
...
if __name__ == "__main__":
    ...
    try:
        ...
        save_dir = Path("/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_1channel_multipFFT_phaseonly")
