...
class DynamicStackDataset(Dataset):
    """
    Loads Phase stacks and Original Real image paths specified in mapping_fixed.json within root_dir.
    Supported JSON structure: 
    "phase.tif": {"phase": "phase.tif", "original": "original.tif"}
    """
    def __init__(self, root_dir: str | Path, originals_dir: str | Path, crop_dim: int | None = None, seed: int = 42):
        ...
        for tel_dir in sorted(self.root_path.iterdir()):
            ...
            for key_file, target_info in tel_fft_map.items():
                # Obtención del nombre del archivo de fase desde mapping_fixed.json
                ...
                # Búsqueda de la imagen original en el directorio asignado
                ...
                try:
                    ...
                    print(f"[ERROR] Failed to read {phase_path.name}: {e}")
                    continue
    ...
    def sample_slice(self, sample_info: dict, n_frames: int = 50, start_idx: int | None = None):
        ...
        Loads Phase for CNN input, and Original Spatial Image to compute FFT2 for Wiener Loss.
        """
        phase_path: Path = sample_info["phase_path"]
        orig_path: Path = sample_info["orig_path"]
        
        # Read complete stack array for phase
        ...
        if orig_path.exists():
            ...
            print(f"Warning: Original file not found at {orig_path}. Falling back to unit magnitude.")
        ...
        # 1-channel input tensor [Phase] -> Shape: (N_frames, 1, H, W)
        input_1ch = phase_tensor.unsqueeze(1)
        
        # Cálculo de la FFT2 compleja directamente sobre la imagen real original normalizada para la Loss
...
if __name__ == "__main__":
    ...
    try:
        ...
        data_path = Path("/scratch/paulabp/TFM/images/images_for_network/FFT/mult_FFTs")
        ...
        save_dir = Path("/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_1channel_multipFFT_phaseonly_fixed_v3")
