...
if __name__ == "__main__":
    ...
    target_n_modes = 119
    model = Network(device=device, n_modes=target_n_modes, n_frames=n_frames_per_epoch, basis_for_wavefront='kl').to(device)
    ...
    save_dir = Path("/scratch/paulabp/TFM/run_outputs_v6_50_sanity_check_instance_norm_annealing")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step, sample_info in enumerate(train_samples_shuffled, start=1):
            ...
            coeff, num, den, psf, otf, train_loss = model(images_norm, images_ft, variance, lengths=lengths, current_modes=current_modes)
        ...
        with torch.no_grad():
            for sample_info in dataset.val_samples:
                ...
                _, _, _, _, _, val_loss = model(val_images_norm, val_images_ft, variance, lengths=lengths, current_modes=current_modes)
                val_epoch_losses.append(val_loss.item())

        val_loss_val = float(np.mean(val_epoch_losses))
        val_loss_history.append(val_loss_val)

        print(f"Epoch {epoch:03d}/{num_epochs} | LR: {scheduler.get_last_lr()[0]:.2e} | Modes: {current_modes}/{target_n_modes} | Train Loss: {train_loss_val:.6e} | Val Loss: {val_loss_val:.6e}")
