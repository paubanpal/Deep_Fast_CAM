...
class ConvBlock(nn.Module):
    def __init__(self, inplanes, outplanes, kernel_size=3, stride=1, bn=True, activation=True):
        ...
        if self.use_bn:
            # In pre-activation, normalization acts on input channels ('inplanes')
...
class DynamicStackDataset(Dataset):
    """
    Loads paired stacks of Magnitude and Phase specified in mapping.json within root_dir.
    ...
    def __init__(self, root_dir: str | Path, crop_dim: int | None = None, seed: int = 42):
        ...
        if mapping_path.exists():
            with open(mapping_path, "r", encoding="utf-8") as f:
                fft_map = json.load(f)
            print(f"[INFO] Loaded mapping.json with {len(fft_map)} mapped telescopes.")
        else:
        ...
        for tel_dir in sorted(self.root_path.iterdir()):
            ...
            # Direct mapping: Key = Magnitude/Module | Value = Phase
        ...
        if len(self.train_samples) == 0:
            raise RuntimeError("[CRITICAL ERROR] The training sample list is empty. Check mapping.json.")
    ...
    def sample_slice(self, sample_info: dict, n_frames: int = 50, start_idx: int | None = None):
        ...
        # Normalize the flux of the input Module by dividing by its mean energy
        ...
        # 2-channel input tensor [Module, Phase] -> Shape: (N_frames, 2, H, W)
        input_2ch = torch.stack([mod_tensor, phase_tensor], dim=1)
        
        # Complex reconstruction applying ifftshift to undo the shift of the DC component to the center
...
class AugmentedDatasetWrapper:
    ...
    def augment(self, sample: dict) -> dict:
        ...
        # Recalculate complex FFT after transformations applying ifftshift
...
def evaluate_reconstruction_and_modes(model_path, data_path, orig_data_path, save_dir, device='cuda'):
    ...
    val_sample_info = dataset.val_samples[1]
    print(f"[INFO] Selected stack: {val_sample_info['module_path'].name}")
    ...
    print("[INFO] Updating telescope basis...")
    ...
    # --- TASK 1: Object Reconstruction (Wiener Filter) and Search for Original Image ---
    print("[INFO] Generating plot and files for the reconstructed object...")
    eps = 1e-6
    ...
    # Apply physical non-negativity (positivity) constraint
    object_reconstructed = np.clip(object_reconstructed_raw, a_min=0, a_max=None)

    # Save isolated reconstructed object separately in TIFF (32-bit float)
    obj_tiff_path = save_dir / "reconstructed_object.tiff"
    tiff.imwrite(obj_tiff_path, object_reconstructed.astype(np.float32))
    print(f"--> Isolated reconstructed object successfully saved to TIFF at: {obj_tiff_path}")

    # Save isolated reconstructed object separately in PNG
    obj_single_png_path = save_dir / "reconstructed_object.png"
    plt.imsave(obj_single_png_path, object_reconstructed, cmap='gray')
    print(f"--> Isolated reconstructed object successfully saved to PNG at: {obj_single_png_path}")

    # Search for corresponding original image stack
    tel_name = val_sample_info["tel_dir_name"]
    mod_filename = val_sample_info["module_path"].name
    
    # Extract base identifier from name (e.g., "CHR181", "wds15245", etc.)
    base_stem = mod_filename.split('_cropped')[0]
    ...
    if matched_orig_path and matched_orig_path.exists():
        print(f"[INFO] Equivalent original image found at: {matched_orig_path}")
        raw_orig_stack = tiff.imread(matched_orig_path)[:50].astype("float32")
        
        # Apply the same center crop if the original image is larger
        ...
        print(f"[WARNING] No direct match found for '{base_stem}' in {orig_tel_dir}. Using mean of module channel.")
        degraded_mean = images_2ch[0, :, 0, :, :].mean(dim=0).cpu().numpy()

    # Save degraded input mean image separately in TIFF (32-bit float)
    mean_tiff_path = save_dir / "degraded_mean_input.tiff"
    tiff.imwrite(mean_tiff_path, degraded_mean.astype(np.float32))
    print(f"--> Degraded input mean successfully saved to TIFF at: {mean_tiff_path}")

    # Save degraded input mean image separately in PNG
    mean_png_path = save_dir / "degraded_mean_input.png"
    plt.imsave(mean_png_path, degraded_mean, cmap='gray')
    print(f"--> Degraded input mean successfully saved to PNG at: {mean_png_path}")

    fig, axes = plt.subplots(1, 2, figsize=(12, 6))
    axes[0].imshow(degraded_mean, cmap='gray')
    axes[0].set_title("Simple Mean (Original Input)")
    axes[0].axis('off')

    axes[1].imshow(object_reconstructed, cmap='gray')
    axes[1].set_title("Reconstructed Object (Wiener)")
    axes[1].axis('off')

    plt.suptitle(f"Reconstruction - Stack: {val_sample_info['module_path'].name}", fontsize=14)
    plt.tight_layout()
    
    obj_plot_path = save_dir / "inspection_reconstructed_object.png"
    plt.savefig(obj_plot_path, dpi=300)
    plt.close()
    print(f"--> Comparative plot successfully saved at: {obj_plot_path}")

    # --- TASK 2: KL Mode Spectrum ---
    print("[INFO] Generating KL decay plot...")
    ...
    plt.plot(mode_indices, mean_abs_coeff, marker='o', markersize=3, color='crimson', label=r'Mean Amplitude $|\alpha_k|$')
    plt.fill_between(mode_indices, mean_abs_coeff - std_coeff, mean_abs_coeff + std_coeff, color='crimson', alpha=0.2, label=r'Deviation ($\sigma$)')
    
    plt.yscale('log')
    plt.xlabel("KL Mode Index")
    plt.ylabel("Coefficient Amplitude (rad)")
    plt.title("Amplitude Spectrum of KL Modes")
    ...
    plt.close()
    print(f"--> KL modes plot successfully saved at: {kl_plot_path}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        output_dir = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_2channels_imageFFT_v4_val[1]/run_outputs_comprobacion_50/plots"
