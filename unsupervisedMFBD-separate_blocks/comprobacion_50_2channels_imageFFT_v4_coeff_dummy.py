...
def evaluate_reconstruction_and_modes(model_path, data_path, save_dir, device='cuda'):
    ...
    with torch.no_grad():
        # --- MODIFICACIÓN: Inferencia Dummy con a_k = 0 ---
        B, Nf = images.shape[0], images.shape[1]
        
        # Generar matriz de ceros para B*Nf secuencias y n_modes (50, 119)
        coeff_corrected = torch.zeros((B * Nf, model.n_modes), dtype=torch.float32, device=device)
        
        # Generar PSFs y OTFs de difracción pura (sin turbulencia)
        psf, psf_ft, wavefront = model.compute_psfs(coeff_corrected)
        psf_ft = rearrange(psf_ft, '(b f) x y -> b f x y', f=Nf)

        # Aplicar el filtro de Wiener usando las OTFs neutras
        num, den, loss = model.loss_and_wiener_filter(images_ft, psf_ft, variance, lengths=lengths)
        coeff = coeff_corrected
...
if __name__ == "__main__":
    ...
    try:
        ...
        output_dir = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_2channels_imageFFT_v4_coeff_dummy/run_outputs_comprobacion_50/plots"
