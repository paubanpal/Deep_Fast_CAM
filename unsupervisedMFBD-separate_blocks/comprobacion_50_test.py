...
def evaluate_reconstruction_and_modes(model_path, data_path, save_dir, device='cuda'):
    ...
    for test_idx, test_sample_info in enumerate(dataset.test_samples):
        print(f"\n[INFO] Procesando stack de test {test_idx + 1}/{len(dataset.test_samples)}: {test_sample_info['path'].name}")
        
        test_sample = dataset.sample_slice(test_sample_info, n_frames=50, start_idx=0)
        images = test_sample["images"].unsqueeze(0).to(device)  # [1, 50, H, W]
        cfg = test_sample["config"]
        H, W = images.shape[-2], images.shape[-1]
        
        # Actualizamos la base dependiendo de la configuración de esta imagen de test específica
        ...
        # Usamos el nombre original sin la extensión para no sobrescribir los guardados
        base_name = test_sample_info['path'].stem  
        ...
        plt.suptitle(f"Reconstrucción - Stack: {test_sample_info['path'].name}", fontsize=14)
        plt.tight_layout()
...
if __name__ == "__main__":
    ...
    try:
        ...
        output_dir = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_test/run_outputs_comprobacion_50/plots"
