...
class DynamicStackDataset(Dataset):
    """
    Manages loading and dynamic slicing of paired random stacks (Images + Precalculated FFTs).
    Uses a mapping.json file in fft_root_dir to match image filenames with FFT filenames.
    """
    def __init__(self, root_dir: str | Path, fft_root_dir: str | Path | None = None, crop_dim: int | None = None, seed: int = 42):
        self.root_path = Path(root_dir)
        self.fft_root_path = Path(fft_root_dir) if fft_root_dir is not None else None
        self.crop_dim = crop_dim
        
        # Cargar diccionario de mapeo si existe ruta de FFTs
        fft_map = {}
        if self.fft_root_path is not None:
            mapping_json_path = self.fft_root_path / "mapping.json"
            if mapping_json_path.exists():
                with open(mapping_json_path, "r", encoding="utf-8") as f:
                    fft_map = json.load(f)
                print(f"[INFO] Se cargó exitosamente mapping.json con {len(fft_map)} telescopios mapeados.")
            else:
                print(f"Warning: No se encontró mapping.json en {self.fft_root_path}. Se intentará coincidencia por nombre exacto.")
        ...
        for tel_dir in sorted(self.root_path.iterdir()):
            ...
            for tiff_path in tiff_files:
                fft_path = None
                if self.fft_root_path is not None:
                    # Buscar la correspondencia en el JSON; si no existe, usar el mismo nombre por defecto
                    fft_filename = tel_fft_map.get(tiff_path.name, tiff_path.name)
                    fft_path = self.fft_root_path / tel_dir.name / fft_filename
                    
                    if not fft_path.exists():
                        print(f"Warning: Missing paired FFT file '{fft_filename}' for {tiff_path.name}. Skipping sample.")
                        continue
    ...
    def sample_slice(self, sample_info: dict, n_frames: int = 50, start_idx: int | None = None):
        """
        Extracts a N-frame contiguous slice from both image and FFT stacks synchronously.
        ...
        fft_path: Path | None = sample_info.get("fft_path")
        ...
        # Read paired FFT slice if available
        fft_frames_tensor = None
        if fft_path is not None:
            raw_fft_data = tiff.imread(fft_path)
            fft_slice = raw_fft_data[start_idx : start_idx + n_frames]
            fft_frames_tensor = torch.tensor(fft_slice, dtype=torch.complex64)
...
if __name__ == "__main__":
    ...
    save_dir = Path("/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm")
