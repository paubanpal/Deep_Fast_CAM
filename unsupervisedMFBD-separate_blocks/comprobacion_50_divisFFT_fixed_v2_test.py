...
def evaluate_reconstruction_and_modes(model_path, data_path, orig_data_path, save_dir, device='cuda'):
    ...
    # 1. Cargar Dataset y obtener los stacks de test
    print("[INFO] Cargando dataset...")
    dataset = DynamicStackDataset(root_dir=data_path, seed=42)
    
    if not dataset.test_samples:
        print("[WARNING] No se encontraron muestras de test. Usando validación como alternativa.")
        test_samples = dataset.val_samples
    else:
        test_samples = dataset.test_samples

    print(f"[INFO] Evaluando {len(test_samples)} stacks del conjunto de test...")

    # 2. Inicializar y Cargar Modelo Entrenado (fuera del bucle para eficiencia)
    ...
    # Iterar sobre cada stack de test
    for idx, test_sample_info in enumerate(test_samples):
        print(f"\n--- Procesando stack de test {idx+1}/{len(test_samples)}: {test_sample_info['module_path'].name} ---")
        ...
        # Crear un subdirectorio único para cada stack para evitar sobreescrituras
        stack_save_dir = save_dir / f"test_stack_{idx}_{test_sample_info['module_path'].stem}"
        ...
        obj_tiff_path = stack_save_dir / "reconstructed_object.tiff"
        ...
        obj_single_png_path = stack_save_dir / "reconstructed_object.png"
        ...
        mean_tiff_path = stack_save_dir / "degraded_mean_input.tiff"
        ...
        mean_png_path = stack_save_dir / "degraded_mean_input.png"
        ...
        obj_plot_path = stack_save_dir / "inspection_reconstructed_object.png"
        ...
        kl_plot_path = stack_save_dir / "inspection_kl_modes_decay.png"
...
if __name__ == "__main__":
    ...
    try:
        ...
        output_dir = "/scratch/paulabp/TFM/run_outputs_v6_50_acc_sched_instance_norm_2channels_divisFFT_fixed_v2_test/run_outputs_comprobacion_50/plots"
