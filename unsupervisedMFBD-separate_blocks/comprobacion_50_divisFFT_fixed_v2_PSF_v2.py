...
def evaluate_reconstruction_and_modes(model_path, data_path, orig_data_path, save_dir, device='cuda'):
    ...
    # psf tiene dimensiones (B * Nf, H, W) -> seleccionamos el fotograma 0
    psf_first_frame = psf[0]

    # Recentrar la componente DC al centro geométrico de la imagen
    ...
    # 3. Guardar en formato PNG escala logarítmica (para visualizar anillos de difracción / speckles débiles)
    ...
    # Extraer el primer fotograma degradado (canal módulo)
    first_degraded_frame = images_2ch[0, 0, 0, :, :].cpu().numpy()

    fig, axes = plt.subplots(1, 3, figsize=(18, 6))

    axes[0].imshow(first_degraded_frame, cmap='gray')
    axes[0].set_title("Primer Fotograma Degradado (Frame 0)")
    axes[0].axis('off')
    ...
    plt.suptitle(f"Frame 0 y Reconstrucción - Stack: {val_sample_info['module_path'].name}", fontsize=14)
    plt.tight_layout()
