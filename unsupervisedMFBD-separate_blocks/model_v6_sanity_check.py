...
if __name__ == "__main__":
    ...
    # Sanity Check: Desactivar augmentor por completo
    augmentor = AugmentedDatasetWrapper(zoom_prob=0.0, zoom_range=(1.05, 1.25))

    # -------------------------------------------------------------
    # Configuración de Sanity Check (Overfitting en 1 Stack)
    # -------------------------------------------------------------
    accumulation_steps = 1  # 1 paso para ver la respuesta directa del gradiente
    
    # Seleccionar un único stack fijo para entrenamiento
    single_train_sample = [dataset.train_samples[0]]
    single_val_sample = [dataset.val_samples[0]]
    ...
    # LR ligeramente más alta (1e-3) para observar una bajada rápida de la pérdida
    ...
    save_dir = Path("/scratch/paulabp/TFM/run_outputs_sanity_check")
    ...
    # -------------------------------------------------------------
    # Epoch Loop: Single Stack Overfitting Test
    # -------------------------------------------------------------
    for epoch in range(1, num_epochs + 1):
        ...
        # Usar únicamente el stack fijado con start_idx=0
        for step, sample_info in enumerate(single_train_sample, start=1):
            ...
            if (step % accumulation_steps == 0) or (step == len(single_train_sample)):
        ...
        # --- VALIDATION PHASE (Evaluación sobre el stack de validación fijado) ---
        model.eval()
        val_epoch_losses = []
        with torch.no_grad():
    ...
    plt.title("Sanity Check: Single Stack Overfitting")
