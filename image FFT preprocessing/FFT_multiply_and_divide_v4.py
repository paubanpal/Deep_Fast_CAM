...
def safe_png_write(target_path, array_data, cmap_name='viridis', title=""):
    """
    Saves a PNG with an interactive colorbar legend using Matplotlib,
    passing through a local temporary file to avoid network save drops.
    """
    # 1. Calculate limits based on colormap type
    ...
    # 2. Render plot with a colorbar using matplotlib figure API
    ...
    ax.axis('off')  # Turn off pixel coordinate axes ticks (optional, remove if you want x/y pixel axes)
    fig.tight_layout()

    # 3. Save via temporary local transfer to network drive
...
output_base_path = input_base_path / "new methods (crops power of 2)"
