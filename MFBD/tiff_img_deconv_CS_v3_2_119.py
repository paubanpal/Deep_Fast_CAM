...
def read_and_deconvolve(path_image, path_folder):
    ...
    for i in n_images:
        ...
        config_path = script_dir / 'config_CS_yaml_2_119.yaml'
        decSI = torchmfbd.Deconvolution(str(config_path))

        # Create patches using size 128
        frames_patches = PyTorchPatchify.patchify(frames[:, :, :], patch_size=256, stride_size=110)
        ...
        obj = []
        frames_back = []
        orig_shape = frames[:, :, :].shape  # [10, 160, 160]
        ...
        # Debug print to your HPC terminal so you can verify the shape is now 160x160
        print(f"--- SUCCESS! Reconstructed Image Shape for {i} frames: {deconv_output.shape} ---")

        # Check if the output is a raw 2D image (160, 160) and expand for uniform plotting loops
        ...
        # 4. Set up the plotting window boundaries based on our real 160x160 shape
        npix = orig_shape[1] 
        ...
        for j in range(2):
            ...
            # Row 1: Deconvolved output (mapped safely to your 160x160 array grid)
            if obj[0].shape[0] > j:
        ...
        # ========================================================
        # NEW ADDITION: Save the actual 160x160 restored image!
        # ========================================================
        # obj[0] contains the numpy array of shape (1, 160, 160)
        # We slice obj[0][0] to get the clean 2D image matrix (160x160)
...
#hpc_base_path = Path("/scratch/paulabp/TFM/images/44_KL_modes/CS/")
#hpc_base_path = Path("/scratch/paulabp/TFM/images/77_KL_modes/CS/")
hpc_base_path = Path("/scratch/paulabp/TFM/images/119_KL_modes/CS/")
star_type = ["dudosas-binarias"]
images_dud_binarias = ["hu874_I_20240228_max_3s_100p_selected_500_cropped_shifted_256.tif"]
