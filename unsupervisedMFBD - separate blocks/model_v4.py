...
class DynamicStackDataset(Dataset):
    ...
    Loads a random stack and extracts a contiguous block of N consecutive frames.
    """
    def __init__(self, root_dir: str | Path, crop_dim: int | None = None):
        ...
        print(f"Discovered {len(self.samples)} stack files across telescope subdirectories.")

    def sample_random_slice(self, n_frames: int = 50):
        """Randomly picks one of the available stacks and extracts a 50-frame contiguous slice."""
        ...
        # Pick random starting index for contiguous sequence
...
if __name__ == "__main__":
    ...
    dataset = DynamicStackDataset(root_dir=data_path)
    ...
    # -------------------------------------------------------------
    # Epoch Loop: Dynamic 50-Frame Random Slicing per Epoch
    # -------------------------------------------------------------
    for epoch in range(1, num_epochs + 1):
        # --- TRAINING PHASE ---
        model.train()
        
        # Pick 1 random stack and sample 50 consecutive frames
        sample = dataset.sample_random_slice(n_frames=n_frames_per_epoch)
        ...
        print(f"Epoch {epoch:03d} | Selected: {sample['filename']} | Slice Frames: [{sample['start_frame']}:{sample['start_frame']+n_frames_per_epoch}]")
        ...
        # --- VALIDATION PHASE (Unaugmented 50-frame slice from a different stack) ---
        model.eval()
        with torch.no_grad():
            val_sample = dataset.sample_random_slice(n_frames=n_frames_per_epoch)
